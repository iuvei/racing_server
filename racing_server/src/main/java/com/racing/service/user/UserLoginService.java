...
public class UserLoginService {
  ...
  public ApiResult clientLogin(String userName, String password, String clientSN) {
    ...
    if (StringUtil.isEmpty(clientSN)) {
      return ApiResult.createErrorReuslt("该客户端暂不可用");
    }
    ...
    if (!user.getClientIsEnable() || user.getClientExpireDate().compareTo(new Date()) <= 0) {
      return ApiResult.createErrorReuslt("该客户端暂时不可用");
    }
    String accessKey = UUIDUtil.getUUIDUpcase();
    String securityKey = UUIDUtil.getUUIDUpcase();
    Date lastLoginTime = new Date();
  ...
  public ApiResult checkClientIsEnable(String clientSN) {
    if (StringUtil.isEmpty(clientSN)) {
      return ApiResult.createErrorReuslt("该客户端暂未在盘口注册");
    }
    User user = this.userRepo.getByClientSN(clientSN);
    if (user == null) {
      return ApiResult.createErrorReuslt("该客户端暂未激活注册");
    }
    ...
    if (user.getClientExpireDate().compareTo(new Date()) <= 0) {
      return ApiResult.createErrorReuslt("该客户端已到期，请联系盘口管理员续费");
    }
    int daySub = DateUtil.daysBetweenTwoDate(user.getClientExpireDate(), new Date());
    if (daySub < 7) {
      return ApiResult.createSuccessReuslt(String.format("您的客户端%s天后到期，请及时续费", daySub));
    }
