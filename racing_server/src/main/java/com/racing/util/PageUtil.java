...
public class PageUtil {
  ...
  /**
   * 获取分页查询时的limit 语句，用于setOrderByClause方法的参数，注意要有排序，同时放到最后边 如：example.setOrderByClause(" id ASC " +
   * page.getLimit());
   *
   * @return
   */
   ...
   * @return
   */
  public static Integer getTotalPage(int totalData, int size) {
    if (size == 0) {
      return 0;
    }
    if (totalData % size == 0) {
